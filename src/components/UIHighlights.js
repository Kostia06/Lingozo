    ...
    <>
      ...
      {!showHighlights && hasSeenTour && !hasCompletedTour && isVisible && (
        <motion.button
          ...
          className="fixed bottom-16 right-4 sm:bottom-20 sm:right-6 z-[45] p-3 bg-gradient-to-r from-green-500 to-emerald-500 rounded-full shadow-2xl hover:shadow-green-500/50 transition-all"
          title="Restart UI Tour"
          aria-label="Restart UI Tour"
        >
      ...
      <AnimatePresence>
        {showHighlights && currentItem && (
          <>
            ...
            {/* Mobile Tooltip - Centered at bottom */}
            <motion.div
              initial={{ opacity: 0, y: 20, scale: 0.9 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: 20, scale: 0.9 }}
              className="md:hidden fixed bottom-0 left-0 right-0 z-[62] p-4 pointer-events-auto"
            >
              <div className="bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-700/50 p-4 max-w-lg mx-auto">
                {/* Header */}
                <div className="flex items-start gap-3 mb-3">
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-r ${currentItem.color} flex items-center justify-center flex-shrink-0`}>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    {currentHighlight === 0 ? (
                      ...
                      <button
                        onClick={() => setCurrentHighlight(currentHighlight - 1)}
                        className="px-3 py-1.5 text-xs bg-gray-800 hover:bg-gray-700 rounded-lg transition-colors text-white"
                      >
                        Back
                      </button>
                    )}
                    ...
                    <button
                      onClick={handleNext}
                      className={`px-4 py-1.5 text-xs bg-gradient-to-r ${currentItem.color} rounded-lg font-semibold transition-all flex items-center gap-1 text-white hover:shadow-lg`}
                    >
                      {currentHighlight === HIGHLIGHTS.length - 1 ? 'Finish' : 'Next'}
                      {currentHighlight < HIGHLIGHTS.length - 1 && (
